...
def main():
    ...
    if "tab" not in st.query_params:
        ...
        st.write(
            "Our mission is unwavering – to remove the hassle from your daily culinary journey and bring joy back to your kitchen."
        )
        st.write(
            "With a simple photo, we unlock the potential of your ingredients and deliver a personalized list of recipes tailored to your preferences."
        )
        ...
        if st.button("Go to Got Meals App"):
            st.write(
                '<meta http-equiv="refresh" content="0; URL=\'?tab=got_meals\'" />',
                unsafe_allow_html=True,
            )
        ...
        uploaded_images = st.file_uploader(
            label="Upload a maximum of 5 items and a minimum of 2 items. Order the images you upload with the first image being the most important.",
            ...
            if submit_button:
                ...
                if ingredient_names:
                    ...
                    recipes = search_recipes(
                        es,
                        lemmatized_ingredient_1,
                        lemmatized_ingredient_2,
                    ...
                    if recipes["hits"]["hits"]:
                        for index, hit in enumerate(recipes["hits"]["hits"], start=1):
                            ...
                            if (
                                ...
                                st.write(
                                    f"Recipe Ingredients: {hit['_source']['ingredients']}"
                                )
                                st.write(
                                    f"Recipe Directions: {hit['_source']['directions']}"
                                )
                                st.write("----------------------------")
                            elif option == "None":
                                st.write(f"--- Recipe {index} ---")
                        ...
                        )  # Leave the output blank if no recipes are found
                else:
                    st.write(
                        "No images uploaded. Please upload at least one image."
                    )  # Inform user if no images are uploaded
        else:
            st.write(
                "No images uploaded. Please upload at least one image."
            )  # Inform user if no images are uploaded
