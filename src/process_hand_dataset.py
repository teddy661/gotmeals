...
CLASSIFICATION_ROOT = pc.data_root_dir.joinpath("separated_training_data")


def main():
    parser = argparse.ArgumentParser(
        description="Parse separated_training_data dataset"
    )
    parser.add_argument(
        "-f",
        dest="force",
    ...
    if not CLASSIFICATION_ROOT.exists():
        ...
        exit(1)
    TRAIN_IMG_DIR = CLASSIFICATION_ROOT

    target_parquet_file = pc.data_root_dir.joinpath("separated_training_data.parquet")
    ...
    for d in directories:
        ...
        for f in files:
            ...
            ImagePath.append(str(f))
    df = pl.DataFrame({"ClassId": ClassId, "ImageId": ImageId, "Image_Path": ImagePath})
    df = parallelize_dataframe(df, read_image_wrapper, num_cpus)
    df = df.with_columns(
        [
            pl.col("ClassId").alias("original_ClassId"),
            pl.col("Width").alias("Scaled_Width"),
            pl.col("Height").alias("Scaled_Height"),
            pl.col("Resolution").alias("Scaled_Resolution"),
            pl.col("Image_Path").alias("Scaled_Image_Path"),
        ]
    )
    print(df.head())
